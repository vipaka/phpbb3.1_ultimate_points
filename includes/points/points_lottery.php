...
* @version $Id: points_lottery.php 838 2012-04-16 04:16:20Z femu $
...
*/
		...
		$template->assign_block_vars('navlinks', array(
			'U_VIEW_FORUM'	=> append_sid("{$phpbb_root_path}points_lottery"),
		...
		if (request_var('purchase_ticket', false) && $user->data['user_id'] != ANONYMOUS)
		{
			...
			{
				$message = sprintf($user->lang['LOTTERY_MAX_TICKETS_REACH'], $points_values['lottery_max_tickets']) . '<br /><br /><a href="' . append_sid("{$phpbb_root_path}points_lottery") . '">&laquo; ' . $user->lang['BACK_TO_PREV'] . '</a>';
				trigger_error($message);
			}
			...
			{
				$message = $user->lang['LOTTERY_NEGATIVE_TICKETS'] . '<br /><br /><a href="' . append_sid("{$phpbb_root_path}points_lottery") . '">&laquo; ' . $user->lang['BACK_TO_PREV'] . '</a>';
				trigger_error($message);
			}
			...
			{
				$message = sprintf($user->lang['LOTTERY_MAX_TICKETS_LEFT'], ($points_values['lottery_max_tickets'] - $number_tickets)) . '<br /><br /><a href="' . append_sid("{$phpbb_root_path}points_lottery") . '">&laquo; ' . $user->lang['BACK_TO_PREV'] . '</a>';
				trigger_error($message);
			}
			...
			{
				$message = $user->lang['LOTTERY_INVALID_INPUT'] . '<br /><br /><a href="' . append_sid("{$phpbb_root_path}points_lottery") . '">&laquo; ' . $user->lang['BACK_TO_PREV'] . '</a>';
				trigger_error($message);
			}
		...
		if ($history_mode == 'points_lottery_history_self' || $history_mode == 'points_lottery_history')
		{
			...
			if ($history_mode == 'points_lottery_history_self')
			{
				$sql_array = array(
				...
				if ($total_wins == 0)
				{
					$message = $user->lang['LOTTERY_NEVER_WON'] . '<br /><br /><a href="' . append_sid("{$phpbb_root_path}points_lottery") . '">&laquo; ' . $user->lang['BACK_TO_PREV'] . '</a>';
					trigger_error($message);
				}

				$template->assign_vars(array(
					'U_VIEW_HISTORY'	=> append_sid("{$phpbb_root_path}points_lottery_history"),
					'U_TRANSFER_USER'	=> append_sid("{$phpbb_root_path}points_transfer"),
					'U_LOGS'    		=> append_sid("{$phpbb_root_path}points_logs"),
					'U_LOTTERY'    		=> append_sid("{$phpbb_root_path}points_lottery"),
					'U_BANK'    		=> append_sid("{$phpbb_root_path}points_bank"),
					'U_ROBBERY'    		=> append_sid("{$phpbb_root_path}points_robbery"),
				));
			}

			// Check, if user is viewing all or own entries
			if ($history_mode == 'points_lottery_history')
			{
				$sql_array = array(
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				$template->assign_vars(array(
					'U_VIEW_HISTORY'		=> append_sid("{$phpbb_root_path}points_lottery_history"),
					'U_VIEW_SELF_HISTORY'	=> append_sid("{$phpbb_root_path}points_lottery_history_self"),
					'U_INFO'				=> append_sid("{$phpbb_root_path}points_info"),
					'U_TRANSFER_USER'  		=> append_sid("{$phpbb_root_path}points_transfer"),
					'U_LOGS'    			=> append_sid("{$phpbb_root_path}points_logs"),
					'U_LOTTERY'    			=> append_sid("{$phpbb_root_path}points_lottery"),
					'U_BANK'    			=> append_sid("{$phpbb_root_path}points_bank"),
			...
			$template->assign_vars(array(
				...
				'U_BACK_TO_LOTTERY'		=> append_sid("{$phpbb_root_path}points_lottery"),
				'U_VIEW_SELF_HISTORY'	=> append_sid("{$phpbb_root_path}points_lottery_history_self"),
				'U_TRANSFER_USER'  		=> append_sid("{$phpbb_root_path}points_transfer"),
				'U_LOGS'    			=> append_sid("{$phpbb_root_path}points_logs"),
				'U_LOTTERY'    			=> append_sid("{$phpbb_root_path}points_lottery"),
