...
* @version $Id: points_points_edit.php 805 2012-02-12 10:31:14Z femu $
		...
		if ($adm_points != false && ($auth->acl_get('a_points') || $auth->acl_get('m_chg_points')))
		{
			...
			else
			{
				...
				$template->assign_vars(array(
					...
					'POINTS_OF_USER'		=> sprintf(number_format_points($row['user_points'])),
					'POINTS_NAME'			=> $config['points_name'],
					'CURRENT_VALUE'			=> $row['user_points'],

					'L_POINTS_MODIFY'		=> sprintf($user->lang['EDIT_POINTS_MODIFY'], $config['points_name']),
					'L_P_POINTS_TITLE'		=> sprintf($user->lang['EDIT_P_POINTS_TITLE'], $config['points_name']),
					'L_USERNAME'			=> $user->lang['USERNAME'],

					'S_ACTION'				=> append_sid("{$phpbb_root_path}points.$phpEx", "mode=points_edit&amp;adm_points=1"),
